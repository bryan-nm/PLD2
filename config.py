...
class OptCfg:
    ...
    # --- objective (instructions 2, 3, 6) ---
    # THE OADM <-> D3PM SLIDER. p_d3pm is the fraction of each batch's rows trained under the D3PM
    # framework: substitution corruption scored by the true KL-ELBO. The rest get absorbing (MASK)
    # corruption scored by the OADM reweighted cross-entropy. 0.0 = pure OADM, 1.0 = pure D3PM,
    # 0.5 = both, which is what "train on both objectives" means here. Unlike ProLoopDiff's `beta`,
    # this switches the LOSS along with the corruption -- beta mixed the corruption while always
    # scoring with the absorbing-state surrogate, so its D3PM half trained against the wrong
    # likelihood. See src/objective.py.
    p_d3pm: float = 0.5
    d3pm_transition: str = "blosum"  # "blosum" (biologically-informed) or "uniform"
    d3pm_T: int = 500                # EvoDiff's T; the Q/Qbar stacks are ~3MB on device
    # Softmax sharpness of the BLOSUM scores before Sinkhorn. NOT the corruption rate -- calibrate_betas
    # owns that -- but how well the base matrix MIXES, which decides whether the chain actually
    # reaches uniform by T. Measured TV(Qbar_T, uniform) at T=500: temp 1.0 -> 0.56 (the ELBO would
    # be dropping a large L_T), 2.0 -> 0.071, 3.0 -> 0.019, 4.0 -> 0.007. The biochemistry survives
    # flattening (A->S, I->V, K->R, W->Y at every temperature), so 3.0 buys a sound stationary
    # assumption for nothing. src/d3pm.py warns if this is ever set somewhere that breaks it.
    d3pm_blosum_temp: float = 3.0
    d3pm_vb_weight: float = 1.0      # weight on the D3PM branch as a whole
    d3pm_ce_weight: float = 1.0      # EvoDiff's lambda. They used 0; see the note in objective.py
    oadm_weight: float = 1.0
    # EOS UPWEIGHTING (instruction 6). One EOS per 512-wide canvas is 0.2% of an unweighted loss,
    # and ProLoopDiff duly learned everything except where to stop (53% of its samples at 181k steps
    # never placed EOS at all). At 20.0 against ~350 residues at 1.0 and ~160 PAD at 0.1, EOS becomes
    # ~5% of the per-sequence loss. Raise it if EOS placement is still weak at 10k steps; the failure
    # mode of raising it too far is over-eager EOS, i.e. short samples, which the length histogram
    # in the eval line makes obvious.
...
if __name__ == "__main__":
    from src.objective import split_sizes
    m = CFG.model_config()
    n_d, n_o = split_sizes(CFG.batch_size, CFG.opt.p_d3pm)
